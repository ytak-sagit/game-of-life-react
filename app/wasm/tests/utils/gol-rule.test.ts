...
import { apply } from "../../build/release";
...
test("2世代の経過後、ブリンカーは元の状態に戻ること", () => {
  ...
  // 3x3
  // biome-ignore format: the array should not be formatted
  const blinker = new Int16Array([
    D, A, D,
    D, A, D,
    D, A, D,
  ]);
  // biome-ignore format: the array should not be formatted
  const expectedAfterTwoGenerations = new Int16Array([
    D, A, D,
    D, A, D,
    D, A, D,
  ]);
...
test("4世代の経過後、グライダーは右下1セル分移動していること", () => {
  ...
  // 4x4
  // biome-ignore format: the array should not be formatted
  const generation1 = new Int16Array([
    D, A, D, D,
    D, D, A, D,
    A, A, A, D,
    D, D, D, D,
  ...
  // biome-ignore format: the array should not be formatted
  const expectedGeneration5 = new Int16Array([
    D, D, D, D,
    D, D, A, D,
    D, D, D, A,
    D, A, A, A,
...
test("apply()実行時間の計測", () => {
  ...
  const width = 1280;
  const height = 800;
  const testData = new Int16Array([...Array(width * height)].map(random));
